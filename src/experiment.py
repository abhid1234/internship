...
def run(model_name: str = DEFAULT_MODEL, sft_epochs: int = 5, opsd_epochs: int = 5):
    ...
    base_student = evaluate(tok, model)                 # patch is untrained ~ base behaviour
    ceiling = evaluate(tok, model, teacher=True)        # same model WITH the playbook in context
    ...
    tok, model = _fresh(model_name)
    train_sft(tok, model, epochs=sft_epochs)
    sft = evaluate(tok, model)
    results["sft"] = sft
    ...
    tok, model = _fresh(model_name)
    train_opsd(tok, model, epochs=opsd_epochs)
    opsd = evaluate(tok, model)
    results["opsd"] = opsd
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    if a.smoke:
        print(">>> SMOKE TEST: tiny model + 1 epoch. Numbers are meaningless; we're just checking nothing crashes.")
        run(SMOKE_MODEL, sft_epochs=1, opsd_epochs=1)
    else:
